...
function hashbuddy_posts_hashtags_filter( $content ) {

	$site_url = trailingslashit( get_bloginfo('url') );

	$pattern = '/([^"\'\&\;\.\+\*\?\$\(\)\{\}\=\<\>\|\:])[#]([\p{L}_0-9a-zA-Z-]+)/iu';

	if ( preg_match_all( $pattern, $content, $hashtags ) ) {

		foreach( $hashtags[0] as $hashtag => $value ) {

			$value = preg_replace('/\s+/', '', $value);

			$pattern = "/". $value  ."/";

			$content = preg_replace( $pattern , ' <a href="' . $site_url . '?s=' . urlencode( $value ) . '" rel="nofollow" class="hashtag">'. htmlspecialchars( $value ) .'</a>', $content );
		}
	}
